...
function renderStep() {
  ...
  if (state.step === 2) {
    const travel = state.route === "travel" ? notice("info", "Если аккаунт раньше использовался в неподдерживаемой стране, после возвращения OpenAI рекомендует очистить кеш и файлы cookie, открыть приватное окно и попробовать другой браузер.") : "";
    ...
    const russiaGuide = state.route === "russia" ? `
      ...
      <div class="selection-summary"><strong>Практический маршрут</strong><span>1. Выберите одну поддерживаемую страну, с которой у вас связан реальный способ оплаты. 2. Подключите стабильный платный VPN к серверу этой страны. 3. Сначала откройте браузерную версию ChatGPT в приватном окне. Не меняйте страны во время регистрации и оплаты.</span></div>
  ...
  if (state.step === 3) {
    const russiaAccount = state.route === "russia"
      ? notice("info", "Создавайте только собственный аккаунт. При ошибке региона не переключайте VPN между странами: откройте приватное окно, очистите файлы cookie и повторите вход через тот же сервер.")
      : "";
  ...
  if (state.step === 7) {
    ...
    const paymentDetails = state.paymentRoute === "foreign-card" ? `
      <div class="selection-summary"><strong>Оплата иностранной картой · пошагово</strong><span>1. Оставьте подключение к той же стране, где выпущена карта. 2. В ChatGPT откройте профиль → «Изменить тариф» (Upgrade Plan) → «Подключить Plus» (Get Plus). 3. Введите данные своей карты и соответствующий платёжный адрес. 4. После оплаты откройте «Настройки» (Settings) → «Аккаунт» (Account) и проверьте, что тариф Plus активен.</span></div>`
      : state.paymentRoute === "app-store" ? `
      <div class="selection-summary"><strong>Оплата через App Store · пошагово</strong><span>1. Войдите в официальное приложение именно в тот аккаунт ChatGPT, которым будете пользоваться. 2. Нажмите «Подключить Plus» (Get Plus). 3. Подтвердите подписку на странице оплаты Apple. 4. Если списание прошло, а тариф не появился: ChatGPT → «Настройки» (Settings) → «Аккаунт» (Account) → «Восстановить покупки» (Restore purchases).</span><a href="https://help.openai.com/en/articles/7905739-chatgpt-ios-app-upgrading-to-a-paid-subscription" target="_blank" rel="noreferrer">Официальный источник OpenAI · на английском ↗</a></div>`
      : state.paymentRoute === "google-play" ? `
      <div class="selection-summary"><strong>Оплата через Google Play · пошагово</strong><span>1. Войдите в официальный ChatGPT под нужным аккаунтом. 2. Выберите «Изменить тариф» (Upgrade) или «Подключить Plus» (Get Plus). 3. Подтвердите подписку через платёжный профиль Google Play выбранной страны. 4. Управляйте подпиской в Google Play → Платежи и подписки → Подписки.</span></div>` : "";
    const russiaPayment = state.route === "russia" && state.plan === "plus" ? `
      ...
      <div class="choice-grid three">
        <button type="button" class="choice-card ${state.paymentRoute === "foreign-card" ? "selected" : ""}" data-payment="foreign-card"><span class="choice-label">Вариант 1</span><strong>Своя иностранная карта</strong><span>Карта выпущена банком поддерживаемой страны; платёжные данные и платёжный адрес совпадают.</span></button>
